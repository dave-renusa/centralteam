#!/usr/bin/env python3
"""
RenUSA Harvest hours dashboard — full-year, interactive.

Pulls all time entries from Harvest v2 for the configured team members from
2026-01-01 through today, writes them to docs/data.json, and generates a
self-contained docs/index.html that includes the data inline plus a
client-side filter/aggregation layer (Chart.js + vanilla JS).

The HTML is meant to be viewed locally: `git pull && open docs/index.html`.
Pages is intentionally disabled because the file contains named-employee data.
...
def resolve_team_ids():
    allow = set(e.lower() for e in TEAM_EMAILS)
    if not allow:
        return None  # no filter
    users = get_all("/users", "users", {"is_active": "true"})
    ids = set()
    for u in users:
        if (u.get("email") or "").lower() in allow:
            ids.add(u.get("id"))
    if not ids:
        die("None of the TEAM_EMAILS matched an active Harvest user.")
    return ids
